...
class CustomerEditPurchaseOrderMainController extends GetxController {
  var imagename = "".obs;
  XFile? image;
  ...
  final textfiled_no_of_bags = TextEditingController();
  ...
  void onInit() {
    ...
    if (MainPresenter.getInstance().userModel.userId != null) {
      ...
      if (Get.arguments != null) {
        ...
        print(' purchase_order_object edit tushar : ${purchase_order_object.customer?.firstName.toString()}');


        selected_transporttype_string.value=purchase_order_object.transportArrangeBy.toString();
        selected_dropdown_string.value=purchase_order_object.plant.toString();
        tempProductList.clear();

        for (var item in itemsList) {
          tempProductList.add(TempProductList(
              productid: item.productId.toString(),
              headerId: (DateTime.now().millisecondsSinceEpoch).toString(),
              brand: item.brand.toString(),
              bird: item.bird.toString(),
              material: item.material.toString(),
              modifiedOn: getDate(),
              noOfBags: item.noOfBags,
              packageingWeight: item.packageingWeight,
              stage: item.stage.toString(),
              totalQuantity:item.totalQuantity)
          );
        }

        print("list size ${tempProductList.length}");
      }
      //tempProductList.clear();
    }
  ...
  void getProductList() {
    ...
    ApiService()
      ...
      if (response.statusCode == 200) {
        ...
        if (response.body != null) {
          final list = jsonDecode(response.body) as List<dynamic>;
          productlist.value = list.map((e) => ProductListResponse.fromJson(e)).toList();
 ...
 /* void updateListAtGivenIndex(int i, String productid, date, String date1) {
  ...
  void removeProductFromTempList(String? productid) {
    ...
    print("new porduct list ${tempProductList.length}");
    update();
  }

  void edtiBages(TempProductList tempproductobject,String? productid) {
   /* tempProductList.removeWhere((item) =>
    (item.productid!.contains(productid as Pattern)));
    ...
    if(validateBagesQty()){
      final object = tempProductList.firstWhere((item) => item.productid == productid);
      object.noOfBags=int.parse(textfiled_no_of_bags.text);
      object.totalQuantity=int.parse(textfiled_no_of_bags.text)*tempproductobject.packageingWeight!;
      update();
      textfiled_no_of_bags.clear();
    }
  }

  bool validateBagesQty(){
    if (textfiled_no_of_bags.text.isEmpty || int.parse(textfiled_no_of_bags.text)==0 ) {
      MainPresenter.getInstance().showErrorToast("Invlid Quantity");
      textfiled_no_of_bags.clear();
      return false;
    } else {
